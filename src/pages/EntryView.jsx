...
export default function EntryView() {
  ...
  return (
    <div className="p-5 sm:p-8 max-w-3xl mx-auto">
      ...
      {files.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-slate-ink mb-3">Files & screenshots</h2>
          <div className="grid grid-cols-2 gap-3">
            {files.map((f) => {
              const isImg = f.file_type?.startsWith('image/') || /\.(png|jpe?g|gif|webp|svg)$/i.test(f.file_name)
              return (
                <div key={f.id} className="bg-paper-warm border border-paper-line rounded-lg overflow-hidden">
                  {isImg && imageUrls[f.id] ? (
                    <button
                      type="button"
                    ...
                    <div className="flex flex-col items-center justify-center h-20 text-slate-soft gap-1 px-2">
                      <span className="text-xs font-mono font-semibold">{f.file_name.split('.').pop().toUpperCase()}</span>
                      <span className="text-xs truncate max-w-full text-center">{f.file_name}</span>
                    </div>
                  ) : null}
