...
export default function EntryEditor() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data: tagsData } = await supabase.from('tags').select('*').eq('user_id', localUserId).order('name')
      setAllTags(tagsData || [])
  ...
  const handleSave = async () => {
    ...
    for (const s of snippets) {
      ...
      if (s.id) {
        ...
        await supabase.from('code_snippets').insert({ entry_id: entryId, user_id: localUserId, language: s.language, code: s.code, description: s.description })
      }
    }
    ...
    for (const f of files) {
      if (f.id) {
        ...
        await supabase.from('entry_files').insert({ entry_id: entryId, user_id: localUserId, file_path: f.file_path, file_name: f.file_name, file_type: f.file_type, caption: f.caption })
      }
    }
  ...
  return (
    <div className="p-5 sm:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6 gap-2">
        <h1 className="font-serif text-xl sm:text-2xl font-semibold text-slate-ink">
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <TagPicker allTags={allTags} selectedIds={tagIds} onChange={setTagIds} userId={localUserId} onTagCreated={(t) => setAllTags([...allTags, t])} />
        </div>
        ...
        <div>
          <label className="block text-xs font-medium text-slate-soft mb-1.5">
            Files &amp; screenshots
            <span className="ml-2 text-slate-soft/70 font-normal">PDF · PPTX · XLSX/XLS · DOC · ZIP · Images</span>
          </label>
          <FileUploader userId={localUserId} entryId={id} files={files} onChange={setFiles} />
        </div>
      </div>
    </div>
  )
