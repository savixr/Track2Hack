  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [{ data: entriesData }, { data: tagsData }, { data: filesData }, { data: snippetsData }] = await Promise.all([
        supabase.from('entries').select('*').eq('user_id', user.id).order('entry_date', { ascending: false }),
        supabase.from('tags').select('*').eq('user_id', user.id),
        supabase.from('entry_files').select('entry_id').eq('user_id', user.id),
        supabase.from('code_snippets').select('entry_id').eq('user_id', user.id),
      ])
      setAllTags(tagsData || [])
