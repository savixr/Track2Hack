  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [{ data: entriesData }, { data: goalsData }] = await Promise.all([
        supabase.from('entries').select('id, entry_date, title, hours_spent, tag_ids').eq('user_id', localUserId).order('entry_date', { ascending: false }),
        supabase.from('goals').select('*').eq('user_id', localUserId).eq('status', 'active').order('created_at', { ascending: false }).limit(3),
      ])
